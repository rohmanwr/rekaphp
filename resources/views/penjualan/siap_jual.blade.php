...
@section('title', 'Barang Siap Jual - Sudah Ready')
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h3 class="fw-bold text-dark mb-1">Barang Siap Jual</h3>
        <p class="text-muted small mb-0">Daftar barang dari Rekap Pembelian dengan status <strong>"Sudah Ready"</strong>.</p>
    </div>
</div>
    ...
    <div class="card-body">
        <form action="{{ route('penjualan.siap-jual') }}" method="GET">
            <div class="input-group">
                ...
                @if(!empty($search))
                <a href="{{ route('penjualan.siap-jual') }}" class="btn btn-outline-secondary"><i class="bi bi-x-lg"></i> Reset</a>
                @endif
            </div>
        </form>
    </div>
...
<div class="card border-0 shadow-sm">
    <div class="card-body p-0">
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    @forelse($siapJualBarangs as $index => $item)
                    <tr>
                        <td class="text-center fw-semibold text-muted">
                            {{ $siapJualBarangs->firstItem() ? $siapJualBarangs->firstItem() + $index : $index + 1 }}
                        </td>
                        <td>
                        ...
                        <td class="text-center">
                            <div class="d-flex justify-content-center gap-1">
                                ...
                                <form action="{{ route('penjualan.destroy-detail-barang', $item->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?')">
                                    @csrf
                                    @method('DELETE')
                        ...
                        <div class="modal-dialog modal-lg">
                            <div class="modal-content text-start">
                                <div class="modal-header">
                                    <h5 class="modal-title fw-bold">Edit Barang Siap Jual ({{ $item->kode_otomatis }})</h5>
                                ...
                                <form action="{{ route('penjualan.update-detail-barang', $item->id) }}" method="POST" enctype="multipart/form-data" class="form-pembelian">
                                    ...
                                    <div class="modal-footer">
                                        ...
                                        <button type="submit" class="btn btn-warning text-white fw-semibold">Update Data</button>
                                    </div>
                                </form>
                            </div>
                        </div>
                    ...
                    @empty
                    <tr>
                        <td colspan="9" class="text-center py-4 text-muted">Belum ada barang berstatus "Sudah Ready".</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    @if($siapJualBarangs->hasPages())
    <div class="card-footer bg-white">
        {{ $siapJualBarangs->appends(['search' => $search])->links() }}
    </div>
    @endif
</div>
